...
export function AiConcierge({ organizationName, userName, geminiEnabled }: { organizationName: string; userName: string; geminiEnabled: boolean }) {
  ...
  return (
    <div className="grid min-h-[calc(100svh-4.5rem)] xl:grid-cols-[1fr_300px]">
      <div className="flex min-h-[calc(100svh-4.5rem)] flex-col">
        <div className="flex items-center justify-between gap-3 border-b bg-white/90 px-4 py-3.5 backdrop-blur-xl sm:px-7">
          ...
          <div className="flex items-center gap-2"><Badge variant="outline" className="hidden rounded-full bg-muted/40 text-[9px] sm:inline-flex">{geminiEnabled ? "Gemini connected" : "Local briefing mode"}</Badge><Button variant="outline" size="sm" className="shrink-0" onClick={() => { setMessages([]); setThreadId(undefined); }}><Plus className="size-3.5" /> <span className="hidden min-[420px]:inline">New thread</span></Button></div>
        </div>

        <div className="flex-1 overflow-y-auto px-4 py-8 sm:px-8">
          <div className="mx-auto w-full max-w-3xl">
            {messages.length === 0 ? (
              <div className="flex min-h-[min(540px,62svh)] flex-col justify-center py-8 sm:py-10">
                ...
                <h2 className="mt-6 text-center text-2xl font-semibold tracking-[-0.045em] sm:text-3xl">What are we solving, {userName.split(" ")[0]}?</h2>
              ...
              <div className="space-y-7">
                {messages.map((message, index) => (
                  <div key={index} className={cn("flex gap-3", message.role === "user" && "justify-end")}>
                    ...
                    <div className={cn("max-w-[92%] rounded-2xl px-4 py-3 sm:max-w-[88%]", message.role === "user" ? "bg-brand-navy text-white" : "border bg-white text-muted-foreground shadow-sm")}><RichText content={message.content} /></div>
                  </div>
                ))}
        ...
        <div className="sticky bottom-[5.4rem] border-t bg-background/90 px-3 py-3 backdrop-blur-xl sm:px-8 sm:py-4 md:bottom-0">
          <form ref={formRef} onSubmit={(event) => { event.preventDefault(); ask(input); }} className="mx-auto max-w-3xl">
            <div className="rounded-2xl border bg-white p-2 shadow-[0_18px_55px_rgba(31,39,48,0.1)] focus-within:border-foreground/25">
              ...
              <div className="flex items-center justify-between px-1 pt-1"><span className="hidden text-[9px] text-muted-foreground sm:inline">Enter to send · Shift + Enter for new line</span><Button type="submit" size="icon" className="rounded-xl" disabled={pending || !input.trim()}><ArrowUp className="size-4" /></Button></div>
            </div>
      ...
      <aside className="hidden border-l bg-white xl:block">
        <div className="p-5">
          ...
          <div className="mt-7 rounded-xl border bg-muted/20 p-4"><p className="text-[10px] font-semibold uppercase tracking-[0.15em] text-muted-foreground">Data boundary</p><p className="mt-2 text-[11px] leading-5 text-muted-foreground">Answers use current organization-scoped records and approved knowledge only.</p></div>
        </div>
      </aside>
    </div>
  );
