...
module Admin
  class NotificationsControllerTest < ActionDispatch::IntegrationTest
    ...
    test "open marks notification as read and redirects to linked record" do
      sign_in @admin

      get open_admin_notification_url(@notification)

      assert_redirected_to admin_project_path(@project)
      assert_not_nil @notification.reload.read_at
    end

    test "open handles absolute admin urls on current host" do
      sign_in @admin
      @notification.update!(url: "http://www.example.com#{admin_project_path(@project)}")
    ...
    test "return_to accepts absolute admin urls on current host" do
      sign_in @admin

      patch admin_notification_url(@notification, return_to: "http://www.example.com#{admin_project_path(@project)}")
